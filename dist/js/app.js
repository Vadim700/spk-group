(() => {
    ...
    menuHeader.addEventListener("click", (event => {
        ...
        if (target.parentElement.classList.contains("menu__item")) {
            const menuSubList = target.parentElement.lastElementChild;
            for (let item of menuSubList.children) heightElement += item.clientHeight + parseInt(window.getComputedStyle(item).marginBottom);
            heightElement += 2 * parseInt(window.getComputedStyle(menuSubList).paddingTop);
        ...
        if (target.parentElement.classList.contains("menu__subitem")) {
            const menuSubSublist = target.parentElement.lastElementChild.lastElementChild;
            console.log(menuSubSublist.cons);
            for (let item of menuSubSublist.children) heightElement += item.clientHeight + parseInt(window.getComputedStyle(item).marginBottom);
            heightElement += 2 * parseInt(window.getComputedStyle(menuSubSublist).paddingTop);
